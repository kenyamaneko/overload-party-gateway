...
func (h *Handler) HandleUpgrade(c *gin.Context) {
	...
	if h.authClient != nil {
		...
		if err != nil || player == nil {
			slog.Warn("ws handler: find player by firebase uid failed", "firebase_uid", decoded.UID, "error", err)
		...
		if err != nil || player == nil {
			slog.Warn("ws handler (dev): player not found", "firebase_uid", uid, "error", err)
			c.JSON(http.StatusUnauthorized, gin.H{"error": "player not registered"})
			return
		}
		playerID = player.PlayerID
