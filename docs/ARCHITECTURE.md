...
本ドキュメントは **コードを読んでも一見しては分からない設計意図** だけを残す。実装詳細（タイマーの起動順、map のキー構造、env 一覧、各メッセージの JSON 形状）は各ファイルの実装とコメントを一次情報とする。
...
- フラグは不成立の申告 (「成立通知が届かないマッチの扱い」) にもかかる。申告 API は同じ matchId を二度受けても壊れないため、重複を許容できない成立通知の側だけをフラグで守る

## 成立通知が届かないマッチの扱い

成立通知の宛先が接続していない場合、ペアはキューから取り出されたまま matchmaking 側に残る。gateway は 1 人ずつ成立を通知し、届かなかったプレイヤーが居た場合はマッチの不成立を matchmaking へ申告する。どちらにも届かなかった場合も、ペアは成立の時点でキューから取り出されているため申告する。

- 接続が残る側には `error` (code: `matchmaking_error`, retryable: `true`) を送る。専用の種別を足すとクライアントと `ws-constants` の両側に追随が要る
- 失敗の通知は申告より先に送る。申告が失敗しても待機中のプレイヤーが再試行できるようにするため
- 申告に失敗した回はエラーを返すが、再配信は「成立通知の一度きりの送出」のフラグで止まるため申告には到達しない。申告は 1 回だけ試みるものとして扱う
...
- match_made: push は接続を持つプロセスに届く。受け取ったプロセスが「自分に該当プレイヤーの接続があるか」だけ見る。接続が無いプレイヤーが居た場合は「成立通知が届かないマッチの扱い」に従って不成立を申告する

同一 playerID の重複接続は単一接続契約で旧接続を close する。
...
決着した対戦は、続けて消費したバトル回数を両プレイヤーに戻す。返却が成功した対戦だけを返却済みとして記録するので、失敗した対戦は次の起動でやり直される。account は同じ対戦への返却を 1 回だけ適用するため、記録の書き込みに失敗してやり直しても回数は二重に戻らない。

記録がある対戦は入室を断り、切断猶予の評価（[BDR-002](https://github.com/kenyamaneko/overload-party-common/blob/main/docs/bdr/002-disconnect-resolution.md)）の対象からも外す。プレイヤー自身の切断で切れた対戦は記録の対象にならないため、この 2 つの経路は混ざらない。回数を戻すのは停止が原因の対戦だけで、この区別が崩れると切断を繰り返して回数制限を回避できてしまう。

対象は人間 2 人の対戦に限る。対戦相手が居ない対戦には切断猶予の評価が働かず、停止をまたいでもそのまま再開できるため。account の返却は異なる 2 つのプレイヤー ID を要求するので、NPC 戦の消費バトル回数は戻さない。
