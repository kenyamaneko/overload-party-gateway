...
サービス概要・起動手順は [../README.md](../README.md)、REST エンドポイントは [API_REFERENCE.md](API_REFERENCE.md)（自動生成）、WS プロトコルは [WS_REFERENCE.md](WS_REFERENCE.md)、DB スキーマは [DATA_DESIGN.md](DATA_DESIGN.md) を参照。

## 1. Gateway の責務境界

gateway はクライアント ⇄ 下流サービス間のプロトコル終端とセッション整合のみを担い、ドメイン状態・ルール判定・永続化されるドメインデータを所有しない。保持するのは WS 接続 map、ゲーム/観戦セッションのインメモリ索引、`gateway.game_players` テーブル（match_made 起点の冪等キー用途）、各種タイマー。
...
## 2. battle の passive 設計に起因する gateway 側 orchestration
...
| battle イベントへのプレイヤー名・レベル補完 | battle の `GameStateView` は name/level を持たない（battle が account のスキーマ変更に引きずられないため）。WS 向けに合成するのは WS を持つ gateway |
...
クライアント → gateway 入り口で Firebase ID Token を検証する（gateway）。検証済み FirebaseUID を accountclient 経由で PlayerID に解決した後、下流サービス（ClusterIP のみに公開）は URL パスの `playerId` を無検証で信頼する。

この信頼チェーンは **下流を VPC 外に公開しない** ことが前提。下流サービスを将来外部公開する場合は、gateway と同等のトークン検証を各下流に入れるか、mTLS/サービス間認証トークンを導入しないと境界が崩れる。

`middleware.FirebaseAuth` → `middleware.PlayerResolve` のチェーンが REST / WS 両入口で使われ、`playerID` を context に載せる。ハンドラは context の `playerID` だけを信頼し、リクエストボディから PlayerID を取らない（クライアント側成りすまし防止）。

## 4. match_made の二重ゲーム作成防止（多層冪等性）
...
## 7. `gateway.game_players` テーブルの役割

gateway がドメイン状態を持たないと言いつつ 1 つだけ DB テーブルを持っている理由:

- **EXP 付与の冪等キー** (§5): インメモリ dedup は Pod 再起動で消えるため、永続化が必要
...
どちらも「WS session 境界の冪等性・低レイテンシ要件」に由来する。battle にこれを寄せると、battle が WS 概念を持ち込むことになり §2 の分業が崩れる。
